...
guint64 dump_table_data(MYSQL *conn, FILE *file, char *database, char *table, char *where, char *filename)
{
	...
	while ((row = mysql_fetch_row(result)))
	{
		...
		if (!statement->len)
		{
			...
			const char *altered_table_name = table;
			if (strcmp(table, "qing_outlinkdoc") == 0)
			{
				altered_table_name = "light_link_tidb";
			}
			if (strcmp(table, "qing_invite_link") == 0)
			{
				altered_table_name = "invite_link_tidb";
			}
			if (strcmp(table, "qing_link_member") == 0)
			{
				altered_table_name = "light_link_member_tidb";
			}
			...
			if (complete_insert || has_generated_fields)
			{
				if (insert_ignore)
				{
					g_string_printf(statement, "INSERT IGNORE INTO `%s` (", altered_table_name);
				}
				else
				{
					g_string_printf(statement, "INSERT INTO `%s` (", altered_table_name);
				}
			...
			else
			{
				if (insert_ignore)
				{
					g_string_printf(statement, "INSERT IGNORE INTO `%s` VALUES", altered_table_name);
				}
				else
				{
					g_string_printf(statement, "INSERT INTO `%s` VALUES", altered_table_name);
				}
			}
			num_rows_st = 0;
		}
		...
		for (i = 0; i < num_fields; i++)
		{
			// convert sid to int64
			if ((strcmp(fields[i].name, "sid") == 0 || strcmp(fields[i].name, "linkid") == 0) && (fields[i].flags & BINARY_FLAG))
			{
				g_string_erase(sid, 0, sid->len);
				bool ok = parseSID(row[i], sid);

				if (!ok)
				{
					// printf("Met an error. Keep going.");
					metError = true;
				}
				break;
			}
		...
		for (i = 0; i < num_fields; i++)
		{
			...
			{
				// convert sid to int64
				if (strcmp(fields[i].name, "sid") == 0 || strcmp(fields[i].name, "linkid") == 0)
				{
					if (fields[i].flags & BINARY_FLAG)
					{
						g_string_append(statement_row, sid->str);
